<template>
  <div>
    <form @submit.prevent="checkHomeEligibility" class="apply-for-loan-submit">
      <div class="col-md-12 ">
      ...
      <div class="form-group" >
        <label >Full Name (পুরো নাম)</label>
        <input
          type="text"
          class="form-control"
      ...
      <div class="form-group">
        <label for="dob" class="">Date of Birth (জন্ম তারিখ)</label>
        <datepicker
      ...
      <div class="form-group">
        <label for="houseNo">Apartment/House No. (অ্যাপার্টমেন্ট / বাড়ি নম্বর)</label>
      ...
      <div class="form-group">
        <label for="roadNo">Road No. (রোড নম্বর)</label>
        <input
          type="text"
          class="form-control"
      ...
      <div class="form-group">
        <label for="areaLocation">Area / Location (ঠিকানা)</label>
        <input
          type="text"
          class="form-control"
      ...
      <div class="form-group">
        <label for="branchName">Nearest IDLC Branch (নিকটতম আইডিএলসি শাখা)</label>
        <select
          class="form-control"
      ...
      <div class="form-group">
        <label for="phone">Contact Number (যোগাযোগের নম্বর)</label>
        <input
          type="number"
          class="form-control"
      ...
      <div class="form-group">
        <label for="email">Your Email (ইমেইল)</label>
        <input
          type="email"
          class="form-control"
      ...
      <div v-if="isIndependentProfessionals">
        ...
        <div class="form-group">
          <label for="loanType">Please select your major profession </label>
          <select
            type="text"
            class="form-control"
...
<!--      Hidden isLandlord-->
      ...
      <div class="form-group" >
        <label for="monthlyIncomeFromOtherSources">Monthly income from other sources if any (অন্যান্য উত্স থেকে মাসিক আয়, যদি থাকে)</label>
        <select
          type="text"
          class="form-control"
      ...
      <div class="form-group">
        <label for="requestedLoanAmount">Requested Loan Amount (লোনের পরিমাণ)</label>
        <input
          type="number"
          class="form-control"
        ...
        <div class="error_message" v-if="submitted && !$v.formData.requestedLoanAmount.between">Won't accept any amount less than BDT 500,000/- </div>
      </div>

      <div class="form-group">
        <label for="loanType">What Type of Loan do you need?  (আপনার কোন ধরণের লোন প্রয়োজন?)</label>
        <select
          type="text"
          class="form-control"
          :class="{ 'is-invalid': submitted && $v.formData.loanType.$error }"
          v-model="formData.loanType"
          data-placement="right"
          @change="onChangeLoanType($event)"
        >
          <option value="" disabled > Select a loan type</option>
          <option value="Apartment-Purchase-Loan" >Apartment Purchase Loan</option>
          <option value="House-Purchase-Loan" >House  Purchase Loan</option>
          <option value="Commercial-Space-Loan" >Commercial/Shop Space Loan</option>
          <option value="Construction-Loan" > Construction Loan</option>
          <option value="Group-Construction-Loan" >Group-Construction Loan</option>
          <option value="Home-Equity-Loan" > Home Equity Loan</option>
          <option value="Loan-Transfer-from-other-bank-FI" selected>Loan Transfer from other bank / FI</option>
          <option value="Registration-Purpose-Loan" >Registration Purpose Loan</option>
          <option value="Semi-Paka-or-1-storied-Paka-House">Semi Paka or 1 storied Paka House Loan </option>
          <option value="Land-purchase-with-structure">Land purchase with structure </option>
        </select>
        <div class="error_message" v-if="submitted && !$v.formData.loanType.required">This field is required.</div>
      </div>

      <!--        // Hidden Tenure of your loan-->
      <div class="form-group">
        <label for="tenure">Tenure of your loan Years (লোনের মেয়াদ) </label>
        <input
          type="number"
          class="form-control"
      ...
      <div class="form-group">
        <label for="doYouHaveAnyExistingLoan">Do you have any existing loan? (আপনার কি কোনও বিদ্যমান লোন আছে?)</label>
        <select
          type="text"
          class="form-control"
  ...
  export default {
    ...
    validations: {
      formData: {
        ...
        loanType: { required },
        tenure: { required, numeric, between: between(1, 25)},
    ...
    methods: {
          ...
          if (this.getAge <= 25 || this.getAge >= 60){
            ...
            if (this.isServiceHolder == true){
              ...
              if(this.formData.majorProfessionForNRB == 'Service-holder') {
                if (totalMonthlyIncome <= 80000) {
                  let message = "Not working for both service & businessman-NRB.";
                  this.warningMessage(message);
                }
              }else{
      ...
      onChangeLoanType(event){
        this.formData.maxTenure = null;
       let selectedLoanType = event.target.value;
        selectedLoanType =  this.formData.loanType;
        if (selectedLoanType == 'Apartment-Purchase-Loan'
          || selectedLoanType == 'House-Purchase-Loan'
          || selectedLoanType =='Construction-Loan'
          || selectedLoanType =='Group-Construction-Loan'){
          this.maxTenure = 25;
          this.interestRate = 9;
        }else if (selectedLoanType == 'Home-Equity-Loan'
          || selectedLoanType == 'Loan-Transfer-from-other-bank-FI'
          || selectedLoanType == 'Semi-Paka-or-1-storied-Paka-House'){
          this.maxTenure = 15;
          this.interestRate = 10;
        } else if (selectedLoanType == 'Registration-Purpose-Loan'){
          this.maxTenure = 12;
          this.interestRate = 10;
        }else if (selectedLoanType == 'Land-purchase-with-structure'
          || selectedLoanType == 'Commercial-Space-Loan'){
          this.maxTenure = 12;
      ...
      submitDataForHomeLoan(){
        ...
        data.append('loanType', this.formData.loanType);
