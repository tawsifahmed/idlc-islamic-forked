        ...
        <div class="row filterOptions gReport">
            <div class="col-md-12">
                <b-card class="product-card">
                    <a v-b-toggle="'annualReportcollapse' + 1" block class="d-flex cursor_pointer">
                        <strong
                    ...
                    <div v-bind:id="'annualReportcollapse' + 1">
                        <b-card class="question-card">
                            <b-card v-if="videos.length <= 0" class="question-card loading">
                            ...
                            <div class="row" v-else>
                                <div class="latest_news slider multiple-items col-md-12">
                                    <div class="row videos">
                                        <div
                                        v-for="(video, index) in videos" :key="`learningMaterial_${index}`" class="col-lg-3 col-ms-6 col-12"
                                        >
                                        <LazyYoutube
                                            class="iframe"
                                ...
                                <div class="more_video" v-if="videos.length > 0 && showViewMore">
                                        <div @click="handleClickViewMoreVideo">View More</div>
                                    </div>
                            </div>
                        </b-card>
                      </div>
                ...
                <b-card class="product-card">
                    ...
                    <div v-bind:id="'annualReportcollapse' + 2">
                        <b-card class="question-card">
                            <b-card v-if="pdfs.length <= 0" class="question-card loading">
                                <div>No Content Available</div>
                            </b-card>
                            <div v-else class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div
                                        v-for="file in pdfs"
                                        :key="file.id"
                                        class="col-lg-3 col-ms-6 col-12"
                                    >
                                    ...
                                    <div class="more" v-if="pdfs.length > 0 && showViewMorePdf">
                                        <div @click="handleClickViewMorePdf">View More</div>
                                    </div>
                                </div>
                            </div>
                        </b-card>
...
export default {
    data(){
        return{
            ...
            pdfs: [],
            showViewMore: false,
            showViewMorePdf: false,
        };
    },
    methods: {
        handleClickViewMoreVideo(){
          axios.get('get-learning-material').then(res => {
            const result = res.data.learningMaterials
            this.videos = []
            result.filter(item => {
              if(item.category_id === "video") {
        ...
        handleClickViewMorePdf(){
          axios.get('get-learning-material').then(res => {
            const result = res.data.learningMaterials
            this.pdfs = []
            result.filter(item => {
              if(item.category_id === "pdf") {
                this.pdfs.push(item)
              }
            })
            this.showViewMorePdf = false
          })
        },
        getTitlteData() {
            axios.get('get-learning-material-title').then(res => {
                this.title = res.data.data.title
                this.sub = res.data.data.subtitle
            }).catch(error => {
                console.log(error)
            })
        },
        getLearningMaterial() {
            axios.get('get-learning-material').then(res => {
                this.learningMaterials = res.data.learningMaterials
                if(this.learningMaterials.length > 8) {
                  this.learningMaterials.filter(item => {
                    if(item.category_id === "video") {
                      this.videos.push(item)
                      this.videos = this.videos.slice(0, 8)
                      this.showViewMore = true
                    }else {
                      this.pdfs.push(item)
                      this.pdfs = this.pdfs.slice(0, 8)
